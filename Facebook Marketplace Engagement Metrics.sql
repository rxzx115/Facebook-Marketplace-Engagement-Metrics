...
SELECT *,
  date(status_published) AS date_calc,
  EXTRACT(YEAR FROM status_published) as year_calc,
  time(status_published) AS time_calc,
  EXTRACT(HOUR FROM status_published) as hour_calc
FROM molten-thought-441320-u6.Example.facebookmarketplaceengagement
...
-- To create a report of the number of listings, comments, shares, etc. by year
...
SELECT
  year_calc, 
  COUNT(status_id) AS status_id_count,
  SUM(num_comments) AS num_comments_total,
  SUM(num_shares) AS num_shares_total,
  SUM(num_reactions) AS num_reactions_total
FROM calculatedcte
GROUP BY year_calc
ORDER BY year_calc ASC


-- To create a report of the number of listings, comments, shares, etc. by month in YTD 2018
SELECT
    EXTRACT(MONTH FROM status_published) AS month_calc,
    COUNT(status_id) AS status_id_count,
    SUM(num_comments) AS num_comments_total,
    SUM(num_shares) AS num_shares_total,
    SUM(num_reactions) AS num_reactions_total
FROM
    molten-thought-441320-u6.Example.facebookmarketplaceengagement
WHERE
    EXTRACT(YEAR FROM status_published) = 2018
GROUP BY
    month_calc
ORDER BY
    month_calc ASC
